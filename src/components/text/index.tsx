...
export function Text({
    ...
    function RenderWithSpace({ t }: { t: string }) {
        return (
            <>
                {t
                    .split(/\n?\r?\\n/)
                    .map((line: string, index: number, array: string[]) => (
                        <div key={line} className="inline">
                            {line}
    ...
    function RenderItalics({ t }: { t: string }) {
        const tlist = t.split("_");
                        ...
                        {tlist.map((t: string, index: number) => (
                            <>
                                {index % 2 == 0 ? (
                                    ...
                                    <span
                                        key={index}
                                        className="inline italic text-accent-color"
                                    >
                                        <RenderWithSpace t={t} />
                                    </span>
                                )}
    ...
    function RenderBold({ t }: { t: string }) {
        const tlist = t.split("*");
        const hasList = tlist.length > 1 ? true : false;
        return (
            <>
